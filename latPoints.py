...
def latpts(basis = np.array([[0,0,0]]) , plv = np.array([[1,0,0],[0,1,0],[0,0,1]]) , bounds = np.array([1,1,1]) , plane = False):
	plvc = np.array([[1,0,0],[0,1,0],[0,0,1]])
	latpoints = {}
	...
	if not plane :
		...
		while not bnds :
			if np.array_equal(plv, np.array([[0.5,0.5,0],[0,0.5,0.5],[0.5,0,0.5]])) :
				plv = np.append(plv, [[1,0,0],[0,1,0],[0,0,1]], axis=0 )
			...
			for i in basis[...,]:
				...
				if i[0]*100 + i[1]*10 + i[2] == bounds[0]*100 + bounds[1]*10 + bounds[2] :
					bnds = True
					break
